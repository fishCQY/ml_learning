...
# 编码器设计​​
# ​​层级压缩​​：通过5层stride=2的卷积逐步缩小特征图尺寸（64→32→16→8→4→4）
# ​​通道扩张​​：每层通道数翻倍（dec_channels*1 → *2 → *4 → *8 → *16）
# ​​批归一化​​：每层卷积后接BN加速收敛
# ​​激活函数​​：LeakyReLU（slope=0.2）缓解梯度消失
# ​​2. 解码器设计​​
# ​​上采样策略​​：使用插值（interpolate） + 填充（pad） + 卷积代替转置卷积
# ​​插值方法​​：最近邻插值（速度快，保留边缘）
# ​​填充逻辑​​：pad=(2,1,2,1)表示左右各填充2和1，上下同理，用于调整尺寸
# ​​卷积计算​​：使用普通卷积（kernel=4）处理上采样后的特征
# ​​渐进式重建​​：通过4次上采样逐步恢复分辨率（4→8→16→32→64）
# ​​输出处理​​：最后使用Sigmoid将像素值压缩到[0,1]
# ​​3. 潜在空间处理​​
# ​​维度压缩​​：将最终4x4x(dec*16)特征展平后通过全连接层映射到latent_size
# ​​维度恢复​​：解码时通过全连接层恢复到4x4x(dec*16)
